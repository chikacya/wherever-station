...
Raw, Base64, Anywhere, and Loon outputs preserve any value shaped like `<scheme>://...`. A URI with an unknown scheme is stored and emitted without semantic rewriting. This makes it possible to pass through protocols such as `sudoku://`, `snell://`, or a future scheme to a client that understands it.
...
Mihomo YAML, sing-box JSON, and Surge configuration require semantic conversion. Wherever Station emits a node only when its adapter can represent the relevant URI parameters. Unsupported protocols or parameter combinations are reported as skipped rather than guessed.
...
sing-box rejects fallback/load-balancing groups and the implicit China-direct policy rather than changing their behavior. Surge accepts select and URL-test groups; ambiguous delimiter-bearing values are rejected. VMess conversion never implicitly disables certificate verification. Check subscription preflight before distributing a structured output.
