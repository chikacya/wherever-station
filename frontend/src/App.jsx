...
function ManagedNowhereDeploy({ state, setState, clients, me, notify, persist, onNavigate, onDiscover }) {
  ...
  const selectNowhereBinarySource = async (source) => {
    update("binarySource", source);
    if (source !== "download") return;
    const releases = await loadNowhereReleases();
    setForm((current) => ({
      ...current,
      binarySource: "download",
      version: releases.some((release) => release.tag === current.version)
        ? current.version
        : releases.find((release) => nowhereVersionCapabilities(release.tag).verified)?.tag || current.version || "v2.1.0",
    }));
  };
  ...
  return <section className="workspace-page deploy-panel">
    ...
    <Modal open={singEditor} title="新建 sing-box 节点" eyebrow="快捷部署" onClose={() => !singCreating && setSingEditor(false)} size="large">
      ...
      <div className="form-grid">
        <Field label="内核来源"><select value={singForm.binarySource || "copy"} onChange={async event => { const value = event.target.value; updateSing("binarySource", value); if (value === "download") { const releases = await loadSingBoxReleases(); setSingForm((current) => ({ ...current, downloadVersion: releases.some((release) => release.tag === current.downloadVersion) ? current.downloadVersion : releases[0]?.tag || current.downloadVersion || "1.13.11" })); } }}><option value="copy">复制目标机现有内核</option><option value="download">下载官方发行版</option></select></Field>
