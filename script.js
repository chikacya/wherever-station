...
function cleanIpProfile(value) {
  ...
  return {
    ...
    risk: { score: value.risk && typeof value.risk.score === "number" && Number.isFinite(value.risk.score) ? value.risk.score : null, level: ["low", "medium", "high", "unknown"].includes(value.risk && value.risk.level) ? value.risk.level : "unknown", proxy: cleanText(value.risk && value.risk.proxy, 16), residential: typeof (value.risk && value.risk.residential) === "boolean" ? value.risk.residential : null },
    purity: { score: value.purity && value.purity.score !== null && Number.isFinite(Number(value.purity.score)) ? Math.max(0, Math.min(100, Number(value.purity.score))) : null, label: cleanText(value.purity && value.purity.label, 24) || "待判断", confidence: ["high", "medium", "low"].includes(value.purity && value.purity.confidence) ? value.purity.confidence : "low", confidenceScore: Math.max(0, Math.min(100, Number(value.purity && value.purity.confidenceScore) || 0)), sourceCount: Math.max(0, Math.min(8, Number(value.purity && value.purity.sourceCount) || 0)), sources: (Array.isArray(value.purity && value.purity.sources) ? value.purity.sources : []).slice(0, 8).map((item) => cleanText(item, 32)).filter(Boolean), networkClass: cleanText(value.purity && value.purity.networkClass, 32) || "待判断", proxyDetected: typeof (value.purity && value.purity.proxyDetected) === "boolean" ? value.purity.proxyDetected : null },
...
function cleanState(input) {
  ...
  const subscriptions = (Array.isArray(input.subscriptions) ? input.subscriptions : []).map((item) => {
    ...
    const quota = { mode: rawQuota.mode === "machine" ? "machine" : "none" };
...
function subscriptionChangePreview(params) {
  ...
  if (JSON.stringify(before.quota) !== JSON.stringify(after.quota)) fields.push(`服务器流量展示：${after.quota.mode === "machine" ? "开启" : "关闭"}`);
...
function subscriptionTraffic(state, subscription) {
  let traffic = { upload: 0, download: 0, total: 0, expire: 0, observedAt: "" };
  ...
  entry.promise = (async () => {
    ...
    try {
      const request = async () => {
        ...
        if (!serverTraffic(machine, latest)) return null;
        let records = [];
        if (machine.trafficPlan?.enabled) {
          try {
            const response = await server.call('common:getRecords', { uuid: machine.monitorClientId, type: 'load', load_type: 'network', start: new Date(cycleStart(machine.trafficPlan.resetDay) - 20 * 60000).toISOString(), end: new Date().toISOString(), maxCount: 4000 });
            records = response?.records?.[machine.monitorClientId] || [];
          } catch (_) { /* Fall back to labelled Agent totals, without quota. */ }
        }
        return serverTraffic(machine, latest, records);
      };
...
function publicSubscription(req, res) {
  try {
    ...
    const finish = (sample) => {
      const traffic = sample ? { ...sample, expire: availability.traffic.expire } : availability.traffic;
      const userinfo = subscriptionUserinfo(traffic); if (userinfo) res.setHeader("Subscription-Userinfo", userinfo);
      if (sample) res.setHeader('X-Wherever-Traffic-Scope', `server; basis=${sample.basis}`);
    ...
    if (subscription.quota.mode === 'machine') return machineSubscriptionTraffic(state, subscription).then(finish);
    finish(null);
