...
assert.throws(() => methods.get("proxyConsole:previewManagedNowhere")({ input: { ...managedInput, version: "v1.8.3" } }), /尚未通过/);
...
const failedEdit = methods.get('proxyConsole:prepareManagedNowhereUpdate')({ instanceId: managedDefaults.id, readOperationId: readSpec.operationId, changes: { port: 53077, tcpPort: 53077, udpPort: 53077 } });
...
const edit = methods.get('proxyConsole:prepareManagedNowhereUpdate')({ instanceId: managedDefaults.id, readOperationId: readSpec.operationId, changes: { name: 'Renamed Nowhere', port: 53077, tcpPort: 53077, udpPort: 53077 } });
const editedPlan = require('../tools/managed-nowhere').planManagedNowhere({ ...managedInput, name: 'Renamed Nowhere', port: 53077, tcpPort: 53077, udpPort: 53077 });
...
assert.throws(() => methods.get("proxyConsole:prepareManagedNowhereAction")({ instanceId: managedDefaults.id, action: "migrate-v2", targetVersion: "v2.0.2" }), /不支持/);
...
assert.equal(state.managedInstances.find(item => item.id === singDefaults.id).status, "running");
const withoutManagedNode = JSON.parse(JSON.stringify(state));
withoutManagedNode.nodes = withoutManagedNode.nodes.filter(item => item.id !== renamedNode.id);
assert.throws(() => methods.get("proxyConsole:saveState")({ state: withoutManagedNode }), /托管节点/);
const withoutManagedInstance = JSON.parse(JSON.stringify(state));
withoutManagedInstance.managedInstances = withoutManagedInstance.managedInstances.filter(item => item.id !== singDefaults.id);
assert.throws(() => methods.get("proxyConsole:saveState")({ state: withoutManagedInstance }), /部署节点页面/);
const withoutSourceNode = JSON.parse(JSON.stringify(state));
withoutSourceNode.nodes = withoutSourceNode.nodes.filter(item => item.id !== airportNode.id);
assert.throws(() => methods.get("proxyConsole:saveState")({ state: withoutSourceNode }), /外部订阅源/);
assert.throws(() => methods.get("proxyConsole:deleteNodes")({ nodeIds: [renamedNode.id], expectedRevision: state.revision }), /部署节点页面/);
assert.throws(() => methods.get("proxyConsole:deleteNodes")({ nodeIds: [airportNode.id], expectedRevision: state.revision }), /外部订阅源/);
const disposableManual = { id: "delete-manual-test", name: "可删除手动节点", protocol: "vless", machineId: "", uri: "vless://00000000-0000-4000-8000-000000000009@delete.example.com:443?security=tls#Delete", enabled: true, tags: [], source: "manual", sourceId: "" };
state = methods.get("proxyConsole:saveState")({ state: { ...state, nodes: [...state.nodes, disposableManual] } });
state = methods.get("proxyConsole:deleteNodes")({ nodeIds: [disposableManual.id], expectedRevision: state.revision });
assert(!state.nodes.some(item => item.id === disposableManual.id));
...
assert.equal(quotaResponse.headers["Subscription-Userinfo"], undefined);
assert.equal(state.subscriptions.find(item => item.id === "test-mixed").quota.mode, "none");
...
const quotaPreview = methods.get("proxyConsole:previewSubscriptionChange")({ subscription: { ...state.subscriptions.find((item) => item.id === "test-mixed"), quota: { mode: "machine" } } });
assert(quotaPreview.fields.some((item) => item.startsWith("服务器流量展示：")), "subscription change preview must disclose quota changes");
