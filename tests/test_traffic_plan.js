...
(async () => {
  ...
  const result = await methods.get("proxyConsole:saveMachineTrafficPlan")({
    machineId: "machine-1",
  ...
  failSync = true;
  await assert.rejects(methods.get("proxyConsole:saveMachineTrafficPlan")({ machineId: "machine-1", revision: result.state.revision, plan: { enabled: true, limitBytes: 50, accounting: "down", resetDay: 1, warningLevels: [70, 90, 100] } }), /komari unavailable/);
  ...
  const disabled = await methods.get("proxyConsole:saveMachineTrafficPlan")({ machineId: "machine-1", revision: result.state.revision, plan: { ...result.state.machines.find((item) => item.id === "machine-1").trafficPlan, enabled: false } });
